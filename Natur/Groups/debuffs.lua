...
function Natur_ReadPlayerDebuffs()
	...
	if(Natur_AccountOptions[112]) then   
...
--
--	Clear ALL debuff entries in the table.
--
function Natur_ClearPlayerDebuffs()
	-- this function will clear all debuffs stored for a player (i.e. when i die)
	Natur_TargetDebuffs = {};
	NaturPlayersDebuffsGroup:UnregisterAllBars();
	NaturPrimaryGroup:UnregisterAllBars();
	Natur_ReadPlayerBuffs();		-- need to make sure these are put back (for flasks etc)
	Natur_ReadPlayerCooldowns();	-- need to make sure these are put back 
end;