...
-- Version Details.
NATUR_VERSION = "60000-R1";
NATUR_VERDATE = "22/10/2014";
...
Natur_AccountOptions = {
	...
	10, 620, "UP", 15, 250, 4, 10, 1, 9, true,		-- 43x, y, dir, height, width, texture, max bars, font, font size, on?		[PLAYER COOLDOWNS]
	10, 420, "UP", 15, 250, 4, 10, 1, 9, true,		-- 53x, y, dir, height, width, texture, max bars, font, font size, on?		[FRIENDLY CASTS] 
	10, 450, "UP", 15, 250, 4, 10, 1, 9, true,		-- 63x, y, dir, height, width, texture, max bars, font, font size, on?		[TARGET COOLDOWNS] 
	10, 480, "UP", 15, 250, 4, 10, 1, 9, true,		-- 73x, y, dir, height, width, texture, max bars, font, font size, on?		RESERVED
	...
	false,											-- show all casts around me (or just T/F).	** spam heaven **	[113]
	false,											-- Not used ** reserved **									[114]
...
function Natur_OnLoad()
	...
	NaturFrame:RegisterEvent("ADDON_LOADED");	
	NaturFrame:RegisterEvent("PLAYER_DEAD");
	NaturFrame:RegisterEvent("PLAYER_ENTERING_WORLD");
	NaturFrame:RegisterEvent("UNIT_COMBO_POINTS");
	NaturFrame:RegisterEvent("ACTIONBAR_UPDATE_COOLDOWN");
	NaturFrame:RegisterEvent("PLAYER_TARGET_CHANGED");
	NaturFrame:RegisterEvent("COMBAT_LOG_EVENT_UNFILTERED");
	NaturFrame:RegisterEvent("CHAT_MSG_COMBAT_FACTION_CHANGE");
...
function Natur_Commandline(args)
	...
	elseif(lowerargs == "about") then	
		DEFAULT_CHAT_FRAME:AddMessage("|cff00e0ffNatur Enemy Castbar (r"..NATUR_VERSION..") - Programmed by Michael Boyle (www.softrix.co.uk).  You can email me direct on michael.boyle@softrix.co.uk or use the Curse or Wowinterface pages.");
	else
...
function Natur_Initialise()
	...
	NaturLib = LibStub:GetLibrary("Softrix-5.4");
	-- init the groups.
	...
	DEFAULT_CHAT_FRAME:AddMessage("|cff00e0ffNatur Enemy Castbar (r"..NATUR_VERSION.." 22/10/2014) loaded! Type /natur for options.|cffffffff - Programmed by Michael Boyle c/o Softrix Studios @ www.softrix.co.uk.  You can email me direct on michael.boyle@softrix.co.uk or use the Curse or Wowinterface pages.");
...
function Natur_SetAnchor(setting)
	...
	if(Natur_AccountOptions[2]) then
		...
		-- target cooldowns
		if(Natur_AccountOptions[72] or Natur_AccountOptions[137]) then NaturMyCooldownsGroup:SetAnchorVisible(setting);
		else NaturMyCooldownsGroup:SetAnchorVisible(false); end;
		-- player cooldowns
		if(Natur_AccountOptions[52]) then NaturMyCooldownsGroup2:SetAnchorVisible(setting);
		else NaturMyCooldownsGroup2:SetAnchorVisible(false); end;
		-- diminish returns
		...
		-- player buffs
		if(Natur_AccountOptions[102]) then NaturPlayersBuffsGroup:SetAnchorVisible(setting);
		else NaturPlayersBuffsGroup:SetAnchorVisible(false); end;
		-- player debuff
		if(Natur_AccountOptions[112]) then NaturPlayersDebuffsGroup:SetAnchorVisible(setting);
		else NaturPlayersDebuffsGroup:SetAnchorVisible(false); end;
		...
		NaturMyCooldownsGroup2:SetAnchorVisible(false);
...
function Natur_SetPartyStatus()
	-- check if im in a battleground or arena
	...
	if(instanceType == "arena") then	-- ive left out the normal bg (pvp) to stop random spam
		...
		Natur_ChatDestination = "INSTANCE_CHAT";
	-- lets see if im in a party or raid.
	else
		-- read number of people in my group.
		numPlayers = GetNumGroupMembers();
		Natur_IsPlayerInArena = false;
		if(numPlayers > 1 and numPlayers < 6) then
			-- theres only 5 of us, was this a custom party or a LFG?
			_,_,difficulty,_,_,_,_ = GetInstanceInfo();
			if (difficulty > 0) then Natur_ChatDestination = "INSTANCE_CHAT";
			else Natur_ChatDestination = "PARTY"; end;
		elseif(numPlayers > 5) then
			-- im over 5 players, am i a ready made one, or a lfr?
			_,_,difficulty,_,_,_,_ = GetInstanceInfo();
			if (difficulty > 0) then Natur_ChatDestination = "INSTANCE_CHAT";
			else Natur_ChatDestination = "RAID"; end;
		else
...
function Natur_EventHandler(event,...)
	...
	if(event == "PLAYER_DEAD") then 
		-- ive died, need to clear all and refresh.
		Natur_ClearPlayerDebuffs();
		Natur_RemoveAllDiminish();
	...
	if(event == "COMBAT_LOG_EVENT_UNFILTERED" and Natur_PerCharOptions[1]) then
		...
		if(arg2 == "SPELL_CAST_START") then
			...
			if(not(arg4 == UnitGUID("player")) and arg5 and arg4) then
				...
				if(arg4 == UnitGUID("target")) then tftablestat = "target";
				elseif(arg4 == UnitGUID("focus")) then tftablestat = "focus"; end;
				-- Natur_AddCastTracking(fire_time, caster_id, caster_name, caster_flags, spell_id, spell_name, bar_id, target)
				local icon, castdur, isHostile = Natur_AddCastTracking(GetTime(), arg4, Natur_StripServer(arg5), arg6, arg12, arg13, arg12..arg4.."Casting", tftablestat);
					...
					if(targetFocus or Natur_AccountOptions[113]) then
						...
						if(Natur_AccountOptions[2]) then	-- we are split mode.
							if(isHostile) then
								if(Natur_AccountOptions[22]) then	-- do we have the hostile casts on?
									...
									if(targetFocus == Natur_FFocus or targetFocus == Natur_HFocus) then	-- its a focus, include his name
										NaturHostileCastGroup:RegisterBar(arg12..arg4.."Casting", arg13.." ("..Natur_StripServer(arg5)..")", castdur, castdur, icon, targetFocus);
									else
										NaturHostileCastGroup:RegisterBar(arg12..arg4.."Casting", arg13, castdur, castdur, icon, targetFocus);
									end;
								end;
							else   
								if(Natur_AccountOptions[62]) then	-- do we have the friendly casts on?
									...
									if(targetFocus == Natur_FFocus or targetFocus == Natur_HFocus) then
										NaturFriendlyCastGroup:RegisterBar(arg12..arg4.."Casting", arg13.." ("..Natur_StripServer(arg5)..")", castdur, castdur, icon, targetFocus);
									else
										NaturFriendlyCastGroup:RegisterBar(arg12..arg4.."Casting", arg13, castdur, castdur, icon, targetFocus);
									end;
								end;
							end;
						else
							...
							if(isHostile and Natur_AccountOptions[22]) then
								if(targetFocus == Natur_FFocus or targetFocus == Natur_HFocus) then
									NaturPrimaryGroup:RegisterBar(arg12..arg4.."Casting", arg13.." ("..Natur_StripServer(arg5)..")", castdur, castdur, icon, targetFocus);
								else
									NaturPrimaryGroup:RegisterBar(arg12..arg4.."Casting", arg13, castdur, castdur, icon, targetFocus);
								end;
		...
		elseif(arg2 == "SPELL_CAST_SUCCESS" and arg4) then
			local spellsid, cooldown, glyph, cc, dr, warn, icon = Natur_ProcessAbilityFired(arg12, arg4);
			if(arg4 == UnitGUID("player")) then
				Natur_ReadPlayerCooldowns();
				if(Natur_AccountOptions[132]) then 
					if(arg12 == 49576) then	-- deathgrip
				...
				if(cc and Natur_AccountOptions[115]) then
					-- Natur_LogCCTarget(mobsid, mobsname, casterid, castername, firetime, spellname)
					if(Natur_AccountOptions[116]) then Natur_LogCCTarget(arg8, arg9, arg4, arg5, arg1, arg13); end;
					Natur_SetPartyStatus();		-- this should already be set but do again just incase.
					local msg = string.format(NATUR_APPLIED_SPELL, arg13, arg9);
			...
			if(not(arg4 == UnitGUID("player")) and cooldown and Natur_AccountOptions[137]) then
				-- Natur_TrackCooldown(caster_id, caster_name, spell_id, spell_name, fire_time, duration, bar_id, icon, faction)
				if(Natur_IsUnitHostile(arg6)) then
					Natur_TrackCooldown(arg4, arg5, arg12, arg13, GetTime(), cooldown, arg4..arg12.."MobCD", icon, Natur_HTarget);
				else
					Natur_TrackCooldown(arg4, arg5, arg12, arg13, GetTime(), cooldown, arg4..arg12.."MobCD", icon, Natur_FTarget);
				end;
				-- if its my target, run the cooldown check
				if(arg4 == UnitGUID("target") and not(Natur_IsPlayerInArena) and previousGUID == targetsGUID) then 
					Natur_ShowTargetCooldowns(arg4); 
			...
			if(warn and Natur_IsUnitHostile(arg6) and Natur_AccountOptions[124]) then		-- hostile rogue spell, warn me?
				-- Natur_AddDetectedRogue(name, spell)
				Natur_AddDetectedRogue(arg5, arg13);
			end;
			-- does this cast have a diminish return we are interested in?    
			if(dr and Natur_IsUnitPlayer(arg10) and arg4 == UnitGUID("player") and not(arg8 == UnitGUID("player")) or dr and not(arg4 == UnitGUID("player")) and Natur_IsUnitPlayer(arg10) and Natur_AccountOptions[122]) then
				-- Natur_TrackDR(mobs_id, mobs_name, icon, faction, caster, spellid, spellname)	
				if(Natur_IsUnitHostile(arg10)) then	-- this is for a hostile?
					Natur_TrackDR(arg8, Natur_StripServer(arg9), icon, "Hostile", Natur_StripServer(arg5), arg12, arg13);
				else
					Natur_TrackDR(arg8, Natur_StripServer(arg9), icon, "Friendly", Natur_StripServer(arg5), arg12, arg13);
				end;
			end;			
		...
		elseif(arg2 == "SPELL_INTERRUPT") then
			if(Natur_AccountOptions[2]) then
				...
				NaturFriendlyCastGroup:UnregisterBar(arg12..arg4.."Casting");
				NaturHostileCastGroup:UnregisterBar(arg12..arg4.."Casting");
			else
				-- grouped up so this timer will be on the primary group.
				NaturPrimaryGroup:UnregisterBar(arg12..arg4.."Casting");
			end;
		...
		elseif(arg2 == "SPELL_AURA_APPLIED") then
			...
			-- Natur_ProcessAbilityFired(spellid, guid)
			local _, cooldown, _, cc, dr, warn, icon = Natur_ProcessAbilityFired(arg12, arg8);		-- check this, do i need guid of player or mob
			-- process buffs/debuffs on target/focus
			if(arg8 == UnitGUID("target") or arg8 == UnitGUID("focus")) then
				Natur_ReadTargetBuffs();
				Natur_ReadPlayerBuffs();
				Natur_ReadTargetDebuffs();
			elseif(arg8 == UnitGUID("player")) then 
				Natur_ReadPlayerBuffs();
			...
			if(arg4 == UnitGUID("player") and not(arg8 == UnitGUID("player")) and cc and Natur_AccountOptions[115]) then
				-- its my cc, track it.
				-- Natur_LogCCTarget(mobsid, mobsname, casterid, castername, firetime, spellname)
				if(Natur_AccountOptions[116]) then Natur_LogCCTarget(arg8, arg9, arg4, arg5, arg1, arg13); end;
			...
			-- does this cast have a diminish return we are interested in?
			if(dr and Natur_IsUnitPlayer(arg10) and arg4 == UnitGUID("player") and not(arg8 == UnitGUID("player")) or dr and not(arg4 == UnitGUID("player")) and Natur_IsUnitPlayer(arg10) and Natur_AccountOptions[122]) then
				if(Natur_IsUnitHostile(arg10)) then	-- this is for a hostile player target.
				-- Natur_TrackDR(mobs_id, mobs_name, icon, faction, caster, spellid, spellname)
					Natur_TrackDR(arg8, Natur_StripServer(arg9), icon, "Hostile", Natur_StripServer(arg5), arg12, arg13);
				else
					Natur_TrackDR(arg8, Natur_StripServer(arg9), icon, "Friendly", Natur_StripServer(arg5), arg12, arg13);
			...
			if(arg4 == UnitGUID("player")) then Natur_ReadPlayerCooldowns(); end;
		...
		elseif(arg2 == "SPELL_AURA_REMOVED" or arg2 == "SPELL_AURA_DISPELLED") then
			-- grab any information for this spell
			local _, _, _, cc, _, _, _ = Natur_ProcessAbilityFired(arg12, arg8);
			-- process any diminish returns.
			Natur_StartExpireTimer(arg8);
			if(arg8 == UnitGUID("target") or arg8 == UnitGUID("focus") or arg8 == UnitGUID("player")) then
				Natur_ReadTargetBuffs();
				Natur_ReadTargetDebuffs();
				Natur_ReadPlayerBuffs();
				Natur_ReadPlayerDebuffs();
			...
			if(arg8 == UnitGUID("player")) then
				if(Natur_CheckMonitoredAbility(arg13)) then
					-- this is a spell i want notified about.
					local msg = string.format(NATUR_SPELL_FADED, arg13);
			...
			if(cc and Natur_AccountOptions[116]) then
				Natur_AnnounceCCTarget(arg8, arg13);
			end;
		...
		elseif(arg2 == "SPELL_MISSED") then
			...
			if(arg5 == UnitName("player") and cc) then
				...
				if(arg15 == "IMMUNE" and Natur_AccountOptions[119]) then
					-- warn my party or raid that this target is immune.
					Natur_SetPartyStatus();		-- this should already be set but double check anyway.
					-- [Afterlife] is IMMUNE to [Polymorph]
					local msg = string.format(NATUR_SPELL_MISSED1, Natur_StripServer(arg9), arg15, arg13);
		...
		elseif(arg2 == "UNIT_DIED") then
			...
			Natur_RemoveDeadDR(arg8);
			Natur_ReadTargetBuffs();
			Natur_ReadTargetDebuffs();
			Natur_RemoveDeadCasts(arg8);
			Natur_DeleteDeadCooldowns(arg8);
		...
		elseif(arg2 == "PARTY_KILL") then
			...
			if(arg4 == UnitGUID("player")) then Natur_QuakeSounds(arg8); end;
...
--
function Natur_AutoChangeRepBar(chatMessage)
	local s,newFaction,factionName,isHeader,curRepBar;
	s,_,newFaction = string.find(chatMessage, NATUR_REPUTATION_PARSE_TXT);
