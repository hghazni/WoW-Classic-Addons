...
function AutoLoot:OnEvent(e, ...)
	if e == 'PLAYER_LOGIN' then
		local oldEnabled, loadable = select(4,GetAddOnInfo('_!SpeedyLoot'))
		if oldEnabled or loadable == 'DEMAND_LOADED' then
			DisableAddOn('_!SpeedyLoot', true)
			StaticPopup_Show('SPEEDYLOOT_OLD')
		end
...
function SlashCmdList.SPEEDYAUTOLOOT(msg)
	...
	if not cmd or cmd == '' or cmd == 'help' then
		...
		print('  |cffEF6D6D/sal set (SoundID) -|r  |cffFAD1D1Set a Sound (SoundID), Example: /sal set 44321|r')
...
StaticPopupDialogs["SPEEDYLOOT_OLD"] = {
	text = "Old Verison detected.\n\nSafe to delete\n_retail_\\Interface\\AddOns\\|cFFF2CE3C_!SpeedyLoot|r\n\nClick to disable old version.",
	button1 = "Reload UI",
	OnAccept = function()
		ReloadUI()
	end,
